...
class EmployeeFilter(django_filters.FilterSet):
    ...
    is_assigned = ChoiceFilter(
        choices=IS_ASSIGNED_CHOICES,
        empty_label="Select Laptop Status"
    )
    name_contains = CharFilter(
        field_name='emp_name',
        lookup_expr='icontains'
    )
    emp_status = ChoiceFilter(
        choices=EMPLOYEEE_STATUS_CHOICES,
        empty_label="Select Employee Status"
    )
    ...
    def __init__(self, data=None, queryset=None, *, request=None, prefix=None):
        ...
        self.form.fields['lk_emp_id'].widget.attrs.update({
            'placeholder': 'Employee ID'
        })
        self.form.fields['name_contains'].widget.attrs.update({
...
class EmployeeAPIFilter(filters.FilterSet):

    emp_name = filters.CharFilter(lookup_expr='icontains')
